...
class BasicTrainer(nn.Module):
    ...
    def  collect_gaussians(
        ...
        for class_name in self.gaussian_classes.keys():
            if class_name == 'Background' and with_particle:
                gs = self.models[class_name].get_gaussians_with_particles(cam,movement,front_cam,first_frame,dataset_name)
            else:
        ...
        # activated_opacities = gs_dict["_opacities"]
    ...
    def load_state_dict(self, state_dict: dict, load_only_model: bool =True, strict: bool = True):
        ...
        gs_bg = self.models['Background']
        gs_rigid = self.models['RigidNodes']
    ...
    def init_viewer(self, port: int = 8080):
        ...
        # # 创建全白图片，RGB 三个通道值都设置为 255
        white_image = np.ones((height, width, 3), dtype=np.uint8) * 255
        server.scene.set_background_image(
            iio.imread(Path(__file__).parent / "./white_image.png"),
            format="png",
        )
       ...
       # self.server = viser.ViserServer(port=port, verbose=False)
        ...
        # self.server.scene.set_background_image(white_image)
        # dark_mode_v = self.server.gui.add_checkbox("Dark mode", initial_value=False)        
        # self.server.gui.configure_theme(dark_mode=dark_mode_v)
