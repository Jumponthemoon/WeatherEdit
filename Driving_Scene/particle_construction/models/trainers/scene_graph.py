    ...
    def forward(
        ...
        if with_particle:
            gs = self.collect_gaussians(
                cam=processed_cam,movement=movement,front_cam=front_cam,first_frame=first_frame,with_particle=with_particle,image_ids=image_infos["img_idx"].flatten()[0],dataset_name=dataset_name
            )
        else:
            gs = self.collect_gaussians(
                cam=processed_cam,
