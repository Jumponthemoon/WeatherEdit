...
def render(
    ...
    with torch.no_grad():
        ...
        # Initialize movement matrix and pose
        ...
        for i in tqdm(indices, desc=f"rendering {dataset.split}", dynamic_ncols=True):
            ...
            if with_particle:
                ...
                if i % 3 == 0:
                    process_cam = trainer.process_camera(cam_infos, image_infos["img_idx"].flatten()[0]).camtoworlds
                    if i != 0:
                        movement[:3, 3] = process_cam[:3, 3] - prev_pose[:3, 3]
                        movement[:3, :3] = prev_pose[:3, :3].T @ process_cam[:3, :3]
                        first_frame = False
                    
                    prev_pose = process_cam
                results = trainer(image_infos, cam_infos, movement, front_cam, first_frame,with_particle,dataset_name=dataset_name)
            else:
            ...
            if i % 3 == 0:
                rgb_surface = to8b(get_numpy(rgb))
                rgb_surface = pygame.surfarray.make_surface(rgb_surface)
                rgb_surface = pygame.transform.flip(rgb_surface, True, False)

                rgb_surface = pygame.transform.rotate(rgb_surface, 90)  
